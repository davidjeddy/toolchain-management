...
- jq is managed by system_tools as must be present before asdf-vm execution

## [0.75.4] - 2025-08-21

### FIX

- regex in `./libs/jenkins/ContainerImage.groovy` that handles publishing tags to GitLab and the remote container registry

## [0.75.3] - 2025-08-21

### FIX

- $PATH missing `$HOME/.asdf/shims`, tools are not correctly found

## [0.75.2] - 2025-08-21

### FIX

- error "failed to run install callback: exit status 1" by capturing any install failures of `asdf install` with ` || true`. Not the best fix but it works for now
- opsie removal of tools from .tool-version

## [0.75.1] - 2025-08-21

### FIX

- cloud tool AWs CLI install process not uses .tmp as the working dir
- KICS build process now properly removes the $HOME/go dir (sorry golang people, you will have to re-download packages after running an install of TC)
- reset helper removes additional tools dirs from $HOME

## [0.75.0] - 2025-08-18

### FIX

- blastRadiusConstraints() is now simplified and now only checks if changeset spans only one account and one region

## [0.74.1] - 2025-08-19

### ADD

- `hcl2json`, `kubectl`, `helm`, and `flux` CLI tools managed but `asdf`

### FIX

- installation process for `aws` CLI tool to be more universal, less distro locked

## [0.74.0] - 2025-08-14

### FIX

- Manage assets from GitHub via `dra` tool replacing custom shell+curl logic

## [0.72.10] - 2025-08-15

### ADD

- helper script to DM users on Slack when a build fails

## [0.72.9] - 2025-08-18

### FIX

- when kics query library is missing it should be re-installed

## [0.72.8] - 2025-08-14

### FIX

- `jq` is not managed by `asdf-vm`, not the OS package manager

## [0.72.7] - 2025-08-14

### FIX

- Python version to `3.13.2` that is available on for both ARM and X86 architectures

## [0.72.6] - 2025-08-14

### FIX

- `golang` package updated to `1.24.6` for Fedora 42

## [0.72.5] - 2025-08-12

### FIX

- the attempt at creating a resource diagram if the current directory is not an IAC module

## [0.72.4] - 2025-08-05

### ADD

- `terramaid` CLI tool to generate Mermaid diagrams of IaC module resources automatically as part of the pre-push GiT hook

## [0.72.3] - 2025-08-05

### FIX

- maven updated to `3.9.11`
- `asdf-vm` to version `0.15.0` due to a Golang segfault when running on Fedora 42 hosts while executing `asdf plugin add ...`
- sonarscanner updated to `7.2.0.5079`
- trivy updated to `0.63.0`

## [0.72.2] - 2025-07-22

### FIX

- ContainerImage.groovy shared pipeline labels are now more specific
- Example.groovy pipeline usage for ContainerImage.groovy

## [0.72.1] - 2025-07-22

### ADD

- logic to check host OS and prevent installer from working if does not match currently supported version

## [0.72.0] - 2025-07-22

### ADD

- helper script for system upgrades from Fedora 40 to Fedora 42

### FIX

- upstream Golang, Checkov, Python, et al. package versions in support of upgrading to Fedora 42

## [0.71.8] - 2025-07-17

### FIX

- `./libs/jenkins/pipelines/vars/ContainerImage.groovy` errant `)` character which is causing invalid syntax error

## [0.71.7] - 2025-07-16

### FIX

- `./libs/bash/iac_tools.sh` tools now install only if not present and the version do not match

## [0.71.6] - 2025-07-16

### FIX

- `./libs/jenkins/pipelines/vars/ContainerImage.groovy` regex evaluation moved outside the pipeline function scope as per https://groups.google.com/g/jenkinsci-users/c/Dvqkoadvwlk/m/fv5LImmnBAAJ?pli=1

## [0.71.5] - 2025-07-15

### FIX

- `./libs/jenkins/pipelines/vars/ContainerImage.groovy` regex pattern for `params.IMAGE_TAG_VERSION` was not syntax correct

## [0.71.4] - 2025-07-15

### FIX

- `./libs/jenkins/pipelines/vars/ContainerImage.groovy` to only push image and tag repo if on `main` branch and `params.IMAGE_TAG_VERSION` is of SemVer pattern

## [0.71.3] - 2025-07-11

### ADD

- additional documentation regarding package update process
- better error catching and output of the kics build and install process

### FIX

- `golang` version `1.24.4` not yet available on Fedora 40, reverted to `1.23.8`
- `kics` version >= `2.1.9` requires Golang `1.24.x`; but we can only do `1.23.x`. Reverted kics to latest supportable version of `2.1.8`

## [0.71.2] - 2025-07-10

### FIX

- warning regarding terragrunt `hclfmt` deprecation warning

## [0.71.1] - 2025-07-10

### FIX

- Python version to `3.12.10` as  `3.13` is not yet available on Fedora 40

## [0.71.0] - 2025-07-08

### FIX

- Updated tools to latest release versions

## [0.70.0] - 2025-07-08

### FIX

- `./libs/jenkins/pipelines/vars/ContainerImage.groovy` to accept more arguments in support of building daily images of the wl-gc-* family

## [0.67.10] - 2025-07-02

### ADD

- removal of image older than 7 days OR untagged during `post{}` of ContainerImage.groovy

### REMOVE

- check got container image `0.0.0` on ContainerImage.groovy as the value is no longer used

## [0.67.9] - 2025-07-01

### FIX

- `./libs/jenkins/pipelines/vars/ContainerImage.groovy` generation of a container image tag ID when value is not provided (try2)

## [0.67.8] - 2025-06-27

### FIX

- `./libs/jenkins/pipelines/vars/ContainerImage.groovy` generation of a container image tag ID when value is not provided
...
- Container image building platform set to only `linux/amd64` until issue w/ dnf targeting aarch64 (ARM) is corrected

## [0.67.5] - 2025-06-23

### FIX

- to revert to known good `0.67.3`

## [0.67.4] - 2025-06-23

### FIX

- `./libs/jenkins/pipelines/vars/ContainerImage.groovy` again to try and get the datetime working

## [0.67.3] - 2025-06-23

### FIX

- syntax error in `./libs/jenkins/pipelines/vars/ContainerImage.groovy` (part 2)

## [0.67.2] - 2025-06-23

### FIX

- syntax error in `./libs/jenkins/pipelines/vars/ContainerImage.groovy`

## [0.67.1] - 2025-06-23

### FIX

- `./libs/jenkins/pipelines/vars/ContainerImage.groovy` to prevent name collisions when building non-published image tag versions

## [0.67.0] - 2025-06-17

### FIX

- `asdf-vm` to version `0.18.0`
- `syft` bumped to `1.27.1`
- `trivy` bumped to `0.63.0`
- encapsulate check for `.bashrc` with a file check
- missing tool locations in `./libs/bash/reset.sh`
- `./docs/CHANGELOG.md` format, which explains why publishing tags were not being created

### REMOVE

- `./libs/jenkins/pipeline/vars/ContainerImage.groovy` now builds multi-arch images
- `skopeo` as no longer required by down stream projects
- remove `append_if` of `asdf.sh` and `completions/asdf.bash` as no longer needed
  - This must be removed from existing `.toolchainrc` manually
- verbose outputs concerning DIFF_LIST in `./libs/bash/git/common.sh

## [0.66.27] - 2025-06-12

### FIX

- triggering IAC checks via git pre-commit/pre-push hooks
- IAC linting logic more sane, to skip tflint provider checks the `tflint.hcl` override is now required (specifically for shared modules, see terraform-aws-ecr-pull-through-cache for an example)

## [0.66.26] - 2025-05-30

### ADD

- './libs/python/change_aws_profile.py' utility script to change AWS profile easy, without having to remember available profiles' names. 

## [0.66.25] - 2025-05-20

### FIX

- `aws-sso` to version `2.0.1`

## [0.66.24] - 2025-05-13

### FIX

- Revert changes that should not have been part of the previous release

## [0.66.23] - 2025-05-12

### FIX

- `generateDiffList()` now more closely aligns with `deployments` Jenkinsfile logic
- Do not evaluate IAC changes in the `worldline-gc-cicd-build-dev` account for execution via automation (Jenkins)

## [0.66.22] - 2025-05-12

### FIX

- `./libs/bash/ecs_service_port_proxy.sh` to always connect to the specified `$SERVICE_NAME` to prevent from occasionally connecting to non-service containers (e.g. `task-health-check` etc.)

## [0.66.21] - 2025-05-09

### FIX

- golang version to what is currently available for Fedora 40

## [0.66.20] - 2025-05-08

### FIX

- fix blast radius check to skip shared module '.' (single dot)

## [0.66.19] - 2025-05-07

### FIX

- allow multi-module commits (up to 25 modules under the same deployment ID (random string))

## [0.66.18] - 2025-05-07

### ADD

- `openinfraquote` install process

## [0.66.17] - 2025-05-06

### ADD

- `openinfraquote` IAC tool to provide rough cost estimates as part of a code change without needing to egress IAC configuration code

## [0.66.16] - 2025-04-30

### FIX

- checking for CHANGELOG changes when executing semver tagging process

## [0.66.15] - 2025-04-29

### FIX

- `Jenkinsfile` `Changelog Entry` stage to check CHANGELOG diff between current branch and target only if not on the target branch

## [0.66.14] - 2025-04-29

### ADD

- `./libs/jenkins/SharedModule.groovy` to check feature branches include changes to ./docs/CHANGELOG.md

### FIX

- `./libs.bash/iac/sem_ver_release_tagging.sh` to check for changes to ./docs/CHANGELOG.md before tagging a new release
- `./libs/jenkins/SharedModule.groovy` archive step updated to archiveArtifacts
- artifacts are now saved during the `post` phase, no longer the `Compliance & SAST` stage

## [0.66.13] - 2025-04-28

### FIX

- `./libs/bash/git/common.sh` generateSBOM() to set ENV VAR TMPDIR to $HOME/.tmp to work around `device out of storage` error from podman + syft
- `./libs/bash/git/common.sh` autoUpdate() to stop outputting the JSON response and properly parse and evaluate the response string

## [0.66.12] - 2025-04-25

### FIX

- Python updated from `3.12.9` to `3.12.10`

## [0.66.11] - 2025-03-28

### FIX

- disable compliance checking on common config folders (not a terraform or terragrunt module)
- disable creating README on common config folders
- disable generating SBOM on common config folders (non-deployable directories)
