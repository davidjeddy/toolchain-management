#!groovy
/* groovylint-disable CompileStatic, DuplicateMapLiteral, DuplicateStringLiteral, GStringExpressionWithinString, LineLength, MethodParameterTypeRequired, MethodReturnTypeRequired, MethodSize, NestedBlockDepth, NoDef, ParameterCount, UnusedImport */
    ...
    pipeline {
        ...
        stages {
            ...
            stage('Changelog Entry') {
                steps {
                    script {
                        if (env.BRANCH_NAME != gitTargetBranch) { // Check when current branch is not the target branch so we have a diff between branches
                            sh(shellPreamble + '''
                                if [[ $(git diff ''' + gitTargetBranch + ''' --name-only | grep -q "docs/CHANGELOG.md"; echo $?;) != "0" ]]
                                then
                                    printf "ERR: No changes to ./docs/CHANGELOG.md detected, exiting with error.\n"
                                    exit 1
                                fi
                            ''')
                        }
