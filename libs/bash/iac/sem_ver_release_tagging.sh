...
CHANGELOG_PATH="docs/CHANGELOG.md" # DO NOT include a leading `./`
PREV_TARGET="HEAD~1"

if [[ $(git diff $PREV_TARGET --name-only | grep -q "$CHANGELOG_PATH"; echo $?;) != "0" ]]
then
    printf "ERR: No changes to %s detected, exiting with error.\n" "$CHANGELOG_PATH"
    exit 1
fi

# get the message from the CHANGELOG_PATH
