...
function cicdPipelineOverrides() {
    ...
    then
        printf "WARN: Are you logged in as the Jenkins user trying to troubleshoot the pipeline? You MUST 'export CI_JOB_URL=\"some_value\"' to emulate an automated pipeline execution.\n"
        exit 0
    fi
